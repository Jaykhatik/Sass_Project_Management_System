    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Here's what's happening in {workspace.name} today.</p>
        </div>
        
        <button className="flex items-center gap-2 bg-primary text-primary-foreground px-4 py-2 rounded-md font-medium shadow-sm hover:bg-primary/90 transition">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="border rounded-xl p-6 bg-card shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
          ...
          <p className="text-3xl font-bold relative z-10">{workspace.projects.length}</p>
        </div>
        ...
        <div className="border rounded-xl p-6 bg-card shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
          ...
          <p className="text-3xl font-bold relative z-10">{workspace.tasks.length}</p>
        </div>
      </div>
            ...
            {workspace.projects.length === 0 ? (
              ...
              <div className="divide-y">
                {workspace.projects.map((project) => (
                  <div key={project.id} className="p-4 hover:bg-muted/30 transition-colors flex items-center justify-between cursor-pointer">
        ...
        <div className="border rounded-xl overflow-hidden bg-card shadow-sm">
          ...
          <div className="p-0">
            {workspace.tasks.length === 0 ? (
              <div className="p-8 text-center text-muted-foreground text-sm">No upcoming tasks. You're all caught up!</div>
            ) : (
              <div className="divide-y">
                {workspace.tasks.map((task) => (
                  <div key={task.id} className="p-4 hover:bg-muted/30 transition-colors flex items-start gap-3 cursor-pointer">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-2 text-xs text-muted-foreground">
                        ...
                        <span className={`px-1.5 py-0.5 rounded uppercase tracking-wider text-[10px] font-bold ${
