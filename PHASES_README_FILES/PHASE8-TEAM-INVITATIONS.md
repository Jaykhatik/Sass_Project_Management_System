...
- **Frontend Feedback:** Unauthorized attempts immediately revert the UI and generate a popup alert stating: *"Failed to update task. You might not have permission."*
...
4. **Verify:** The task snaps back to its original column, and a browser alert pops up: *"Failed to move task. You might not have permission."*
...
- **Method:** `POST`
- **URL:** `http://localhost:3000/api/invites/YOUR_TOKEN_HERE/accept` *(Replace with the token you copied from Step 3)*
