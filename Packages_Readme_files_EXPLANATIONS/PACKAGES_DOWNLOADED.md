...
This document provides a detailed, simple explanation of all the packages required for Phase 1 (and the core of the app), broken down by what they do and exactly why we need them.
...
*   **What it is:** A smart fetching tool for your frontend. It grabs data from your backend and remembers it (caches it).
*   **Why you need it:** If a user navigates from "Projects" to "My Tasks" and back to "Projects", React Query remembers the projects and shows them instantly without making the user wait for another loading spinner. Your project documentation specifically notes "No Redis" and relies on React Query to keep things like notifications updating in real-time.

### 3. `zustand`
*   **What it is:** A lightweight global "state" manager. It holds information that needs to be accessed from anywhere in your app.
*   **Why you need it:** Let's say a user opens a "Create Task" popup. You need a way to tell the app "Hey, the popup is open" so the screen darkens and the popup shows up. Zustand is the simplest way to store small bits of memory like "Is the sidebar open?", "Which workspace is currently selected?", or "Is dark mode on?" without passing that information down through 10 different files.
...
### 6. `shadcn/ui` (The component library)
